...
function findNotPendingIndex(list) {
    return _commonjsHelpers.__awaiter(this, void 0, void 0, function () {
        ...
        return _commonjsHelpers.__generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, Promise.all(list.map(function (p) { return promiseState(p); }))];
                case 1:
                    states = _a.sent();
                    return [2 /*return*/, states.findIndex(function (state) { return state !== 'pending'; })];
            }
        });
    });
}
    ...
    return _commonjsHelpers.__awaiter(this, void 0, void 0, function () {
        ...
        return _commonjsHelpers.__generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    states = _a.sent();
                    return [2 /*return*/, states.reduce(function (acc, curr, index) {
                            if (curr === 'pending') {
                                acc.push(list[index]);
                            }
                            return acc;
...
function exportQueue(_a) {
    var concurrent = _a.concurrent, handleFetch = _a.handleFetch, handleResponse = _a.handleResponse, fetchOpts = _a.fetchOpts, callbacks = _a.callbacks;
    var urls = [];
    var fetching = [];
    var saving = [];
    function addToQueue(p, queue) {
        var queuePromise = new Promise(function (res, rej) {
            p.then(function (ret) {
                res(ret);
                processQueue();
            })
    ...
    function processQueue() {
        return _commonjsHelpers.__awaiter(this, void 0, void 0, function () {
            var fetchedIndex, fetched, url_1;
            return _commonjsHelpers.__generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, filterNotPending(saving)];
                    case 1:
                        // empty finished saves from saving queue
                        saving = _a.sent();
                        return [4 /*yield*/, findNotPendingIndex(fetching)];
                    case 2:
                        fetchedIndex = _a.sent();
                        _a.label = 3;
                    case 3:
                        if (!(saving.length < concurrent && fetchedIndex !== -1)) return [3 /*break*/, 5];
                        fetched = fetching.splice(fetchedIndex, 1).pop();
                    ...
                    case 5:
                        ...
                        while (fetching.length < concurrent && urls.length) {
                            url_1 = urls.shift();
                            addToQueue(handleFetch(url_1, fetchOpts), fetching);
                        }
                        if (urls.length === 0 && saving.length === 0 && fetching.length === 0) {
                            return [2 /*return*/, callbacks.onDone()];
                        }
                        return [2 /*return*/, urls.length + fetching.length + saving.length];
                }
            });
        });
    }
...
function get_src(attrs) {
    var match = /src\s*=\s*(?:"(.*?)"|'(.*?)'|([^\s>]*))/.exec(attrs);
    return match && (match[1] || match[2] || match[3]);
}
function get_srcset_urls(attrs) {
    var results = [];
    // Note that the srcset allows any ASCII whitespace, including newlines.
    var match = /srcset\s*=\s*(?:"(.*?)"|'(.*?)'|([^\s>]*))/s.exec(attrs);
    if (match) {
        var attr_content = match[1] || match[2] || match[3];
        ...
        var regex = /\s*([^\s,]\S+[^\s,])\s*((?:\d+w)|(?:-?\d+(?:\.\d+)?(?:[eE]-?\d+)?x))?/gm;
        var sub_matches = void 0;
...
function _export(_a) {
    ...
    return _commonjsHelpers.__awaiter(this, void 0, void 0, function () {
        function save(url, status, type, body) {
            ...
            var export_file = path.join(export_dir, file);
        ...
        function handleFetch(url, _a) {
            var timeout = _a.timeout, host = _a.host, host_header = _a.host_header;
            return _commonjsHelpers.__awaiter(this, void 0, void 0, function () {
                var href, timeout_deferred, the_timeout, r;
                return _commonjsHelpers.__generator(this, function (_b) {
                    switch (_b.label) {
                        case 0:
                            href = url.href;
                            timeout_deferred = new Deferred.Deferred();
        ...
        function handleResponse(fetched, fetchOpts) {
            return _commonjsHelpers.__awaiter(this, void 0, void 0, function () {
                var _a, response, url, protocol, host, root, pathname, type, body, _b, range, link, cleaned, base_match, base_href, base, match, pattern, hrefs, element, attrs, _i, hrefs_1, href, url_1, location_1;
                return _commonjsHelpers.__generator(this, function (_c) {
                    switch (_c.label) {
                        case 0: return [4 /*yield*/, fetched];
                        case 1:
                            _a = _c.sent(), response = _a.response, url = _a.url;
                            protocol = fetchOpts.protocol, host = fetchOpts.host, root = fetchOpts.root;
                            pathname = url.pathname;
                            if (pathname !== '/service-worker-index.html') {
                                pathname = pathname.replace(root.pathname, '') || '/';
                            }
                            type = response.headers.get('Content-Type');
                            if (!type.startsWith('text/')) return [3 /*break*/, 3];
                        ...
                        case 5:
                            body = _b;
                            range = ~~(response.status / 100);
                            if (range === 2 && type === 'text/html') {
                                link = httpLinkHeader.parse(response.headers.get('Link') || '');
                                ...
                                if (pathname !== '/service-worker-index.html') {
                                    ...
                                    while (match = pattern.exec(cleaned)) {
                                        ...
                                        for (_i = 0, hrefs_1 = hrefs; _i < hrefs_1.length; _i++) {
                                            href = hrefs_1[_i];
                                            url_1 = resolve(base.href, href);
                                            if (url_1.protocol === protocol && url_1.host === host) {
                                                handle(url_1, fetchOpts, queue.add);
                                            }
                                        }
                                    }
                                }
                            ...
                            if (range === 3) {
                                ...
                                type = 'text/html';
                                body = "<script>window.location.href = \"" + location_1.replace(origin, '') + "\"</script>";
                                handle(resolve(root.href, location_1), fetchOpts, queue.add);
                            }
                            return [2 /*return*/, save(pathname, response.status, type, body)];
                    }
                });
            });
        }
        var defaultPort, port, protocol, host, origin, root, entryPoints, proc, seen, saved, fetchOpts, queue;
        return _commonjsHelpers.__generator(this, function (_m) {
            switch (_m.label) {
                case 0:
                    ...
                    return [4 /*yield*/, Deferred.find(defaultPort)];
                case 1:
                    port = _m.sent();
                    protocol = 'http:';
                    host = "localhost:" + port;
                    ...
                    if (!root.href.endsWith('/'))
                        root.href += '/';
                    entryPoints = entry.split(' ').map(function (entryPoint) {
                        var entry = resolve(origin, basepath + "/" + cleanPath(entryPoint));
                    ...
                    return [2 /*return*/, new Promise(function (res, rej) {
                            queue.setCallback('onDone', function () {
                                proc.kill();
                                res();
                            });
                            Deferred.wait(port).then(function () {
                                try {
                                    ...
                                    var workerUrl = resolve(root.href, 'service-worker-index.html');
